...
/* frees a temporary playing field (pointer array plus every row) */
static VOID FreeTmpField(long **old, int rows)
{  int i;

   if(old == NULL) return;
   for(i=rows;i>0;i--) free(old[i-1]);
   free(old);
}   /* End of FreeTmpField */
...
{  FILEDLG fdg;
   ...
   if(!WinLoadString(hab, (HMODULE)0, iLangBase+IDS_OPEN, MESSAGELEN, szTitle) ||
      !WinLoadString(hab, (HMODULE)0, iLangBase+IDS_OPEN, MESSAGELEN, szButton)||
      !WinLoadString(hab, (HMODULE)0, iLangBase+IDS_FILEOPENEXT, CCHMAXPATH,
		     fdg.szFullFile)){
      ...
      if((hini = PrfOpenProfile(hab,fdg.szFullFile))) {
            ...
            for(j=1;j<co-1;j++) {
               ...
               if( fi[i][j] >3 || fi[i][j] < 0) {
                  ...
                  MessageBox(hwndMain, IDMSG_CANNOTOPENINPUTFILE,
                     (PSZ)NULL,MB_OK | MB_ERROR, FALSE); return;}}
         /*�����-�ਪ�,�஢�ઠ � ��९�᢮����*/
         hu=malloc(sizeof(Runner)*qs);//�뤥����� �����
         ba=malloc(sizeof(Runner)*qs);//�뤥����� �����
         for(i=0;i<qs;i++) {
            ...
            if(ba[i].xLeft   <0 || ba[i].xLeft   > co-1 ||
               ...
               PrfCloseProfile(hini);
               FreeTmpField(fi,ro); free(hu); free(ba);
               MessageBox(hwndMain, IDMSG_CANNOTOPENINPUTFILE,
                  (PSZ)NULL,MB_OK | MB_ERROR, FALSE); return;}}
         ...
         if (ru.xLeft   <0 || ru.xLeft   > co-1 ||
             ...
             MessageBox(hwndMain, IDMSG_CANNOTOPENINPUTFILE,
                (PSZ)NULL,MB_OK | MB_ERROR, FALSE); return;}
/*����� �.�. ��९�᢮����*/
          FreeField();                           /* give the old (current-Row) field back
                                                    before Row/Col are replaced */
          MainY += BmpSize*(Row - ro);
         Col = co; Row = ro; QSize = qs; usLevel = le;
         InitNewGame(1); bNew    = ne; //���� (��) ����塞
...
{  FILEDLG fdg;
   ...
   while(TRUE) {
      fdg.cbSize = sizeof(FILEDLG);
      if(!WinLoadString(hab, 0, iLangBase+IDS_SAVE, MESSAGELEN, szTitle)) {
          MessageBox(hwndMain, IDMSG_CANNOTLOADSTRING,
             (PSZ)NULL, MB_OK | MB_ERROR, TRUE); return FALSE; }
      if(!WinLoadString(hab, 0, iLangBase+IDS_SAVE, MESSAGELEN, szButton)) {
          MessageBox(hwndMain, IDMSG_CANNOTLOADSTRING,
             (PSZ)NULL, MB_OK | MB_ERROR, TRUE); return FALSE; }
      fdg.pszTitle = szTitle;
      fdg.pszOKButton = szButton;
      ...
      fdg.papszFQFilename = (PAPSZ)NULL;
      fdg.ulFQFCount = 0L;
      if(szFullPath[0] == '\0'){//�����塞 ��� �� 㬮�砭��
         if(!WinLoadString(hab, (HMODULE)0, iLangBase+IDS_FILEOPENEXT,
      ...
      if((f=fopen(fdg.szFullFile,"r"))!=NULL) {
         ...
         sT = MessageBox(hwndMain, IDMSG_OVERWRITEFILE,
    		          LangText(IDS_TITLE_WARNING, 40, szWarnT), MB_QUERY | MB_YESNOCANCEL, FALSE);
...
{  CHAR szBuf[MAXNAMEL+TITLESEPARATORLEN+CCHMAXPATH];
   ...
   WinLoadString(hab, (HMODULE)0, iLangBase+IDS_APPNAME, MAXNAMEL, szBuf);
   WinLoadString(hab, (HMODULE)0, iLangBase+IDS_TITLEBARSEPARATOR,
