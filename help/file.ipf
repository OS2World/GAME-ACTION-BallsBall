...
:p.The File menu contains commands that you use to create new,
load, and save games. In addition, it also contains the
command that you use to show High Scores and exit the BallsBall
Game. The following commands appear in the File menu&colon.
:parml tsize=25 break=none.
:pt.New Game
:pd.Creates a new untitled game
:pt.Load Game
:pd.Load old game from file.
:pt.Save
:pd.Saves game to current file
:pt.Save As
:pd.Saves the current game in file using a new name
:pt.Show High Scores
:pd.Shows High Scores
:pt.Exit
...
.*      res = PANEL_FILENEW                                     *
.*--------------------------------------------------------------*/
:h1 res=2220 name=PANEL_FILENEW.New Game
:i2 refid=File.New
:p. You can create a new game in the BallsBall Game by
using the New Game command. Score and Lever sets to zero.
To create a new file, do the following&colon.
:ul.
:li.Select the File menu.
:li.Choose the New Game command.
:eul.
:p. The word "Untitled" appears in the title bar of the window.
...
.*      res = PANEL_FILEOPEN                                    *
.*--------------------------------------------------------------*/
:h1 res=2230 name=PANEL_FILEOPEN.Load Game
:i2 refid=File.Open
:p.You can load game from file by using the Load Game command.
To load game from file, do the following&colon.
:ul.
:li.Select the File menu.
:li.Choose the Load Game command.
:eul.
:p.A dialog box appears, showing you a list of files with *.bbg
extentions in the current directory.
...
:i2 refid=File.Save
:p.Use the Save command to save game to current file.
To save changes to the current file, do the following&colon.
:ul.
:li.Select the File menu.
:li.Choose the Save command.
:eul.
...
:p.Use the Save As command to name and save a game in new file.
:p.Follow these steps to save a new file&colon.
:ul.
:li.Select the File menu.
:li.Choose the Save As command.
:eul.
:p.A dialog box appears, prompting you for a new filename.

.*--------------------------------------------------------------*\
.*  File menu Start/Pause command help panel                    *
.*      res = PANEL_DIRECTION_START                             *
.*--------------------------------------------------------------*/
:h1 res=2270 name=PANEL_DIRECTION_START.Start/Pause
:i2 refid=File.Start/Pause
:p.Use the Start/Pause command to run the game, or stop, if
already runnung. Also you can use Enter or Space keys.

.*--------------------------------------------------------------*\
.*  File menu High Scores command help panel                    *
.*      res = PANEL_HIGHSCORES                                  *
.*--------------------------------------------------------------*/
:h1 res=2260 name=PANEL_HIGHSCORES.High Scores
:i2 refid=File.High Scores
:p.Use the High Scores command to shows dialog with current
High Scores.
...
:i2 refid=File.Exit
:p.You quit the BallsBall Game by using the Exit command or
pressing Esc or Alt+F4 keys. To quit the BallsBall Game, do the
following&colon.
:ul.
:li.Select the File menu.
:li.Choose the Exit command.
:eul.

.***********************************************************************
.*Help for Load  Game Dialog box   (D)
:h1 hide res=4163 scroll=none titlebar=yes group=1 id=A9003.Help for Load  Game
.***********************************************************************
...
:h1 hide res=20500 nosearch noprint.Help for Load  Game
:ol compact.
...
and select the :hp2.Load Game:ehp2. pushbutton to display
...
:dd.:link reftype=hd res=20506 group=25.Load Game pushbutton:elink.
:edl.
.***********************************************************************
...
:p.Type the name of the file you want to load in the
:hp2.File name:ehp2. field
and select the :hp2.Load Game:ehp2. pushbutton.
...
:dd.:link reftype=hd res=20506 group=32.Load Game pushbutton:elink.
:dt.
...
:p.The :hp2.File:ehp2. list displays all the files with *.bbg extentions
in the directory you selected from the :hp2.Directory:ehp2. list.
...
.*Help for Load Game pushbutton  (PU)

:h1 hide res=20506 scroll=none titlebar=yes group=17 nosearch noprint.Help for Load  Game
:link reftype=hd res=20556 vpx=left vpy=top vpcx=100% vpcy=70% scroll=vertical rules=border titlebar=none child auto.
:link reftype=hd res=20650 vpx=left vpy=bottom vpcx=100% vpcy=30% scroll=vertical rules=none titlebar=none child auto.

:h1 hide res=20556 group=44 nosearch.Help for Load  Game
:p.Select the :hp2.Load Game:ehp2. pushbutton to load game from
the file.
:p.For more help, select a topic below.
.*************************************************************************

.*Help for Save Game (PD)
:h1 hide res=4192 id=A4192 nosearch.Help for Save Game
:p.Use :hp2.Save:ehp2. to store the game you play. After the game
is saved in a file, the name of the file added to Window Title instead old name.
:nt.If you are playing a new game, select the :hp2.Save:ehp2. or :hp2.Save
as:ehp2. choices to display the :hp2.Save as:ehp2. pop-up so that you can name
the file you are saving of game. A file must have a title to be saved.
.**************************************************************************


.*Help for Save Game As (PD)
:h1 hide res=4208 id=A4208 nosearch.Help for Save Game as
:p.Use :hp2.Save as:ehp2. to name and save a new game or to save an begun
game under a different name, in a different directory, or on a different disk.
.**************************************************************************


.*Help for Save Game As Dialog box (D)

:h1 hide res=4210 scroll=none titlebar=yes group=100 id=A9004.Help for Save Game as
...
:h1 hide res=26500 nosearch noprint.Help for Save Game as
:ol compact.
:li.Select the down arrow to the right of the :hp2.Drive:ehp2. list to display
all the drives on your system.
:li.Select a drive from the :hp2.Drive:ehp2. list.
...
and select the :hp2.Save Game:ehp2. pushbutton.
:eol.
:p.For specific help, select a topic below.
...
:dd.:link reftype=hd res=26506 group=250.Save Game pushbutton:elink.
:edl.
.***********************************************************************
...
:dd.:link reftype=hd res=26506 group=320.Save Game pushbutton:elink.
...
.*Help for Save Game pushbutton  (PU)

:h1 hide res=26506 scroll=none titlebar=yes group=170 nosearch noprint. Help for Save Game
:link reftype=hd res=26556 vpx=left vpy=top vpcx=100% vpcy=70% scroll=vertical rules=border titlebar=none child auto.
:link reftype=hd res=26650 vpx=left vpy=bottom vpcx=100% vpcy=30% scroll=vertical rules=none titlebar=none child auto.

:h1 hide res=26556 group=440 nosearch. Help for Save Game
:p.Select the :hp2.Save Game:ehp2. pushbutton to save the
game in the file to the drive and directory you selected and with the
